...
def create_step():
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == "Exit":
            ...
            prediction_list = []
            if text:
                prediction_list = [item for item in urls_from_project if text in item]

            window['-LAST-SITE-BOX-'].update(values=prediction_list)
            sel_item = 0
            window['-LAST-SITE-BOX-'].update(set_to_index=sel_item)

            if len(prediction_list) > 0:
                window['-LAST-SITE-BOX-CONTAINER-'].update(visible=True)
            else:
                window['-LAST-SITE-BOX-CONTAINER-'].update(visible=False)
            ...
            prediction_list = []
            if text:
                prediction_list = [item for item in urls_from_project if item.lower().startswith(text)]

            window['-LOGIN-PATH-BOX-'].update(values=prediction_list)
            sel_item = 0
            window['-LOGIN-PATH-BOX-'].update(set_to_index=sel_item)

            if len(prediction_list) > 0:
                window['-LOGIN-PATH-BOX-CONTAINER-'].update(visible=True)
            else:
                window['-LOGIN-PATH-BOX-CONTAINER-'].update(visible=False)
